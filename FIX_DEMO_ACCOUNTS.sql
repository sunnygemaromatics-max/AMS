...
-- ── STEP 0: repair the broken handle_new_user() trigger ─────────────────────
-- Your current trigger does:  approval_status = CASE ... 'approved' ... END
-- A CASE of text literals is type `text`, and Postgres will NOT implicitly
-- cast text -> approval_status (enum). So the trigger throws 42804 on EVERY
-- new user — this also breaks normal app signup, not just demo accounts.
-- This rebuilds it with explicit enum casts. (Safe / idempotent.)
