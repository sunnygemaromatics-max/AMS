  ...
  const importAssets = async (rows: any[], strategy: DuplicateStrategy, snap: Snapshot): Promise<Stats> => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const corePayload: any = {
        ...
        license_key: pick(r, ["license_key", "key", "email_id", "email"])?.toString() || null,
        specifications: pick(r, ["specifications", "specification", "domain", "license_type"])?.toString() || null,
        folder_access_details: pick(r, ["folder_access_details", "folder_access", "folder_details", "shared_folder_access"])?.toString() || null,
        asset_subtype: pick(r, ["asset_subtype", "subtype", "asset_type", "type"]) || null,
