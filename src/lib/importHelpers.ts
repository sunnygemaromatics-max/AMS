...
export async function dryRunSheet(
  ...
  rows.forEach((raw, i) => {
    ...
    if (kind === "employee") {
      ...
      addMapping("email", ["email", "email_id", "work_email"]);
      addMapping("phone", ["phone", "mobile", "contact"]);
      addMapping("designation", ["designation", "title", "job_title"]);
      addMapping("role", ["role", "system_role"]);
      addMapping("reporting_manager", ["reporting_manager", "manager", "reports_to", "line_manager"]);
      addMapping("employee_type", ["employee_type", "emp_type", "type", "employment_type"]);
      addMapping("access_level", ["access_level", "access"]);
      addMapping("emergency_contact", ["emergency_contact", "emergency"]);
      addMapping("secondary_email", ["secondary_email", "personal_email"]);
      addMapping("work_location", ["work_location", "office", "office_location"]);
      addMapping("date_of_joining", ["date_of_joining", "doj", "joining_date", "start_date"]);
